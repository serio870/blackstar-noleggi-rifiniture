...
function bsn_render_finalizza_noleggio_page( $args = [] ) {
    ...
    $logo_url = 'http://www.blackstarservice.it/wp-content/uploads/2021/04/logo-1500px.jpg';
    $logo_src = $logo_url;

    if ( $render_document_only && $pdf_mode ) {
        $logo_data = bsn_get_logo_data_uri( $logo_url );
        if ( ! empty( $logo_data ) ) {
            $logo_src = $logo_data;
        }
    }
...
<?php if ( $render_document_only ) : ?>
        ...
        <head>
            ...
            <style>
                ...
                .bsn-doc-header > div:first-child {
                    width: 45%;
                }
                .bsn-doc-header > div:last-child {
                    width: 55%;
                }
                .bsn-doc-columns > div {
                    width: 50%;
                ...
                <?php if ( $pdf_mode ) : ?>
                /* Questo è il passaggio che cambia il comportamento di page-break/taglio (DOMPDF-friendly). */
                .bsn-pdf-avoid-break { page-break-inside: avoid; }
                .bsn-pdf-allow-break { page-break-inside: auto; overflow: visible; }
                <?php endif; ?>
            </style>
        </head>
        <body>
    ...
    <?php endif; ?>
        ...
        <div id="bsn-documento-finalizzazione" style="background: #fff; padding: 30px; max-width: 21cm; margin: 0 auto; border: 1px solid #ddd;">
            ...
            <div class="bsn-doc-header bsn-pdf-avoid-break" style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px; padding-bottom: 10px;">
            ...
            <div class="bsn-doc-columns bsn-pdf-avoid-break" style="display: flex; gap: 15px; margin-bottom: 15px;">
            ...
            <div class="bsn-doc-section bsn-pdf-allow-break" style="margin-bottom: 15px;">
                ...
                <div class="bsn-pdf-avoid-break" style="margin-top: 10px; text-align: right; font-size: 11px; line-height: 1.6;">
            ...
            <div class="bsn-doc-section bsn-pdf-allow-break" style="margin-bottom: 15px; border: 2px solid #000; padding: 12px; background: #fffbf0;">
                ...
                <?php endif; ?>
                        ...
                        <!-- ===== FIRMA CLIENTE ===== -->
            <div class="bsn-doc-section bsn-pdf-avoid-break" style="margin-bottom: 15px; border: 2px solid #000; padding: 12px;">
            ...
            <!-- ===== OPERATORI ===== -->
            <div class="bsn-doc-section bsn-pdf-avoid-break" style="margin-bottom: 15px; border: 1px solid #000; padding: 12px; background: #f9f9f9;">
...
// [BSN_PDF_EMAIL_START]
/**
 * Ritorna il logo come data-uri base64 per i PDF (fallback: stringa vuota).
 */
function bsn_get_logo_data_uri( $logo_url ) {
    $cached = get_transient( 'bsn_logo_data_uri' );
    if ( ! empty( $cached ) ) {
        return $cached;
    }
    ...
    $response = wp_remote_get( $logo_url, [ 'timeout' => 12 ] );
    ...
    set_transient( 'bsn_logo_data_uri', $data_uri, DAY_IN_SECONDS );
...
/**
 * HTML legacy senza ottimizzazioni PDF (fallback rapido).
 */
function bsn_get_finalizza_document_html_legacy( $noleggio_id ) {
    ob_start();
    bsn_render_finalizza_noleggio_page(
        [
            'render_document_only' => true,
...
function bsn_generate_noleggio_pdf( $noleggio_id, $force = false ) {
    ...
    if ( empty( $html ) ) {
        // Fallback non distruttivo: HTML legacy senza nuove regole PDF.
        $html = bsn_get_finalizza_document_html_legacy( $noleggio_id );
    }
    if ( empty( $html ) ) {
        return false;
    }
